...
def create_app(test_config=None):
  ...
  def add_question():
    ...
    new_catergory = body.get('category', None)
    new_difficulty = body.get('difficulty', None)

    try:
      if new_question is None or new_answer is None or new_catergory is None or new_difficulty is none:
        abort(400)
  ...
  def search_result():
    ...
    try:
      ...
      selection = Question.querry.filter(Question.question.ilike('%{}%'.format(search_term)))
  ...
  def get_questions_based_on_category(category_id):
    try:
      ...
      selection = Question.querry.filter(Question.category == category_id).all()
      if not selection:
        abort(404)

      cuerrnt_questions = paginate(request, selection)

      return jsonify({
        'success': True,
        'questions': cuerrnt_questions,
        'total_questions': len(selection),
  ...
  def play_trivia():
    ...
    newquestions = [q.format() for q in questions if q['question'] not in previous_questions]

    if len(newquestions) > 0:
      question = random.sample(newquestions, 1)[0]
      return jsonify({
        'success': True,
        'question': question
      })
